...
describe("isAuthorized middleware", () => {
    ...
    it("should allow manager role when specified in hasRole", () => {
        // Arrange
        mockResponse.locals = {
            uid: "manager123",
            role: "manager",
    ...
    it("should not allow access when uid does not match and role is insufficient", () => {
        // Arrange
    ...
    it("should handle missing id parameter when allowSameUser is true", () => {
        // Arrange
        ...
        // Assert
        // Should fail because no id to match and role is insufficient
        expect(nextFunction).toHaveBeenCalledWith(
            expect.any(AuthorizationError)
        );
    });
